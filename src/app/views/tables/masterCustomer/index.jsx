...
} from '@/app/store/slices/api/customerSlice'
// ** import utils
...
function MasterCustomerTable() {
    ...
    const { dataTableLKey, deactivateCustomerLKey } = useSelector(state => state.loading)
    ...
    const queryHandler = async query => {
        ...
        setCustomers(
            data?.data?.map(item => ({
                ...item,
                refNo: (
                    <CustomLink href={`/master/customer/view/${item?.id}`} icon>
    ...
    const editHandler = useCallback(id => navigate(`/master/customer/edit/${id}`), [])
    ...
    const onModalConfirmHandleDelete = async () => {
        ...
        let isError = false
        try {
            const response = await deactivateCustomer(removeId)
            dispatch(closeModal())
            message = response?.data?.data?.message || response?.data?.message || 'deactivated customer successfully!'
            setRemoveId(null)
        } catch (error) {
            isError = true
    ...
    const getTemplate = async () => getExcel().unwrap()
    ...
    const handleBulkImport = () => {
        ...
        dispatch(
            openModal({
                ...
                title: <Typography variant='h3'>Add Customers</Typography>
            })
        )
    }
                    ...
                    <ConfirmModal
                        ...
                        message={
                            <>
                                ...
                                <strong>
                                    {customers?.find(customer => customer?.id === removeId)?.name || 'Unknown'}
                                </strong>
                                ?
                            </>
                        }
                        ...
                        isLoading={deactivateCustomerLKey}
                    />
...
export default MasterCustomerTable
