...
import { useCreateBinsMutation } from '@/app/store/slices/api/storageLocationSlice'
import { getCustomSx } from '@/utilities'

const customSx = getCustomSx()

export default function MasterBinForm() {
    const dispatch = useDispatch()
    const [createBins] = useCreateBinsMutation()
    const { createBinsLKey } = useSelector(state => state.loading)
    ...
    const validationSchema = z.object({
        ...
        type: z.string().min(1, 'Bin Type is required'),
        quantity: z.coerce
            .number()
    ...
    const formik = useFormik({
        ...
        onSubmit: async values => {
            ...
            let isError
            try {
                const response = await createBins(values).unwrap()
                ...
                await handleQRGeneration(bins, doc)

                // Save the PDF
                doc.save('bins_qrcodes.pdf')
