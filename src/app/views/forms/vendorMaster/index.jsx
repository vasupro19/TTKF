...
import { Box, Grid, FormControlLabel, Checkbox, Divider, useMediaQuery } from '@mui/material'
...
function VendorMasterForm() {
    ...
    const { updateVendorLKey, createVendorLKey } = useSelector(state => state.loading)
    ...
    const [tabsEnabled, setTabsEnabled] = useState([true, false, false, true])
            ...
            if (activeTab < tabLabels.length - 1) {
                ...
                try {
                    ...
                    message =
                        ...
                        response?.message ||
                        `vendor ${editId && parseInt(editId) ? 'updated' : 'created'} successfully !`
                } catch (error) {
        ...
        },
        {
            ...
            fields: [
                ...
                },
                {
                    ...
                    labelTextSx: { marginRight: '54px !important' }
                },
                {
                    name: 'uid_labels_required',
                    type: 'checkboxLabel',
                    label: 'UID labels required',
                    grid: { xs: 12, sm: 3 },
                    labelPlacement: isMobile ? 'start' : 'top'
                },
                {
    ...
    const getData = async (pinCode, type) => {
        ...
        if (type) {
            ...
            // ---- for ref if cities states needs to be fetched
            // formik.setFieldValue('bill_country_id', {
            //     label: data.data[0].country_name,
            //     value: data.data[0].country_id
            // })
            ...
            // const statesData = await getStates(data.data[0].state_id, type)
            // // setBillingStates(statesData)

            // formik.setFieldValue('bill_state_id', { label: data.data[0].state_name, value: data.data[0].state_id })

            // const citiesData = await getCities(data.data[0].city_id, type)
            // // setBillingStates(statesData)

            // // setBillingCities([...billingCities, { label: data.data[0].city_name, value: data.data[0].city_id }])
            // formik.setFieldValue('bill_city_id', { label: data.data[0].city_name, value: data.data[0].city_id })
        } else {
    ...
    const checkIfErrors = () => {
        let isError = false
        let currentTabKeys = tabsFields[activeTab].fields.map(item => item.name)
            ...
            if (
                ...
                if (name === 'pan_no') {
                    ...
                    const formattedPan = value
                        ...
                        .toUpperCase()
                        .slice(0, 10)
                    formik.setFieldValue('pan_no', formattedPan) // Update formik value
    ...
    const identityCardData = [
        { label: 'Company Name', value: formik.values.name },
    ...
    const getVendorData = async id => {
        ...
        if (data.data?.shipping_term) {
            shipping_term = {
                label: shippingTerms.filter(item => item.value === data.data.shipping_term)[0]?.label || '',
        ...
        formik.setValues({
            ...
            qc_required: !!data.data.qc_required,
            allow_excess_grn: !!data.data?.allow_excess_grn,
            tolerance: data.data?.tolerance || 0
        })
    ...
    useEffect(() => {
        if (editId && parseInt(editId)) getVendorData(editId)
    }, [editId])
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%', position: 'relative' }}>
                        ...
                        <Box sx={{ padding: 2 }}>
                            <FormComponent
                                ...
                                submitting={createVendorLKey || updateVendorLKey}
                                customStyle={{
